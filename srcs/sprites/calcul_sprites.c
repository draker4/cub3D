...
/*   Updated: 2023/02/15 13:55:41 by bperriol         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	cube->sprites.sp_height = abs((int)(SCREEN_HEIGHT / \
	cube->sprites.transf_y));
	cube->sprites.start_y = -cube->sprites.sp_height / 2 + SCREEN_HEIGHT / 2;
	if (cube->sprites.start_y < 0)
		cube->sprites.start_y = 0;
	cube->sprites.end_y = cube->sprites.sp_height / 2 + SCREEN_HEIGHT / 2;
	if (cube->sprites.end_y >= SCREEN_HEIGHT)
		cube->sprites.end_y = SCREEN_HEIGHT - 1;
	cube->sprites.sp_width = abs((int)(SCREEN_HEIGHT / \
	cube->sprites.transf_y));
	cube->sprites.start_x = -cube->sprites.sp_width / 2 + \
	cube->sprites.sp_screen_x;
	if (cube->sprites.start_x < 0)
		cube->sprites.start_x = 0;
	cube->sprites.end_x = cube->sprites.sp_width / 2 + \
	cube->sprites.sp_screen_x;
...
void	draw_pixels_sprites(t_cube *cube, int i)
{
	...
	while (++x < cube->sprites.end_x)
	{
		...
		cube->sprites.transf_y < cube->buffer_z[x])
		{
			...
			{
				cube->sprites.d = y * 256 - SCREEN_HEIGHT * 128 + \
				cube->sprites.sp_height * 128;
